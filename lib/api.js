...
api.getAppInfo = function (appname) {
    ...
    info.config = api.getConfig({app: appname})
...
api.getConfig = function (options) {
    if (!options || options.forceGlobal) return globalConfig

    if (!options.app) return globalConfig

    var app = options.app
    var paths = getAppPaths(app)

    var localConfigPath = path.resolve(paths.workPath, '.podconfig')
    var access, localConfig

    try {
        fs.accessSync(localConfigPath, fs.R_OK)
        ...
        if (e.code === 'ENOENT') {
            ...
            // Throw an error to let user know that it's not readable, but exists.
            access = false
        }
    }

    if (access) {
        localConfig = readJSON(localConfigPath)
    ...
    if (globalConfig.apps[options.app]) {
        return globalConfig.apps[options.app]
    }
}
...
function getAppMainScript (workPath, appname) {
    var config = api.getConfig({app: appname})
    ...
    if (config.script) {
        ...
        // console.log('[' + appname + ']', 'No specified script...')
    }
    // } else {
        // if (config && config.script) {
        //     main = config.apps[appname].script
        // } else if (pkg && pkg.main) {
        //     main = pkg.main
        // }
    // }
...
function prepareConfig (appInfo) {
    ...
    for (var o in appInfo.config) {
        if (appInfo.config.local) {
            if (o == 'script') continue

            // Make sure to resolve the log files.
            if (o == 'out_file' || o == 'error_file') {
                var file = appInfo.config[o]

                // NOTE: There is a known fallback that path.resolve does not
                // cont: expand the home directory. (i.e. path.resolve('~/foo'))
                // cont: This resolves to `$(pwd)/~/foo`.
                var resolvedPath = path.resolve(appInfo.workPath, file)

                appInfo.config[o] = resolvedPath
            }
        }
        if (
            o !== 'port' &&
