...
var tableOptions = {
    ...
    head: ['name', 'commit', 'status', 'port', 'restarts', 'uptime', 'memory', 'CPU']
}
    ...
    list: function () {
        api.listApps(function (err, apps) {
            ...
            if (!apps || !apps.length) {
                ...
                var theme = api.getConfig().theme || ''

                tableOptions.head = tableOptions.head.map(function (field) {
                    switch (theme) {
                        case 'dark': return field
                        default: return field.grey
                    }
                })
